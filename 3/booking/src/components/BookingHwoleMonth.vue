...
export default {
    ...
    mounted() {
        this.getBookingHwoleMonth()
    },
    methods: {
        getBookingHwoleMonth() {

            const today = dayjs(this.toDay);
            const startOfMonth = today.startOf('month').set('hour', 7);
            const endOfMonth = today.endOf('month').add(7, 'hour');
            ...
            const bookings = bookingData.sort((a, b) => new Date(a.startTime) - new Date(b.startTime)).filter((booking) => {
                ...
                return booking.roomId == this.roomId &&
                    (bookingStartDate >= startOfMonth || startOfMonth <= bookingEndDate) &&
                    (endOfMonth >= bookingEndDate || endOfMonth >= bookingStartDate);
            });
